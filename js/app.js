(function () {
  ...
  function callback(res) {
    var outputType = vnjs.store.MINECRAFT.TYPE;
    var outputData = JSON.stringify(vnjs.store.MINECRAFT[outputType], null, 2);
  ...
  vnjs.on("preload", function () {
    return $tpl$k.css("display", "flex");
  });
  var loadProgress = $tpl$k.find('.vnjson-loader__progress');
  ...
  vnjs.on("postload", function () {
    return $tpl$k.fadeOut(300);
  });

  function screen () {
    vnjs.store.screen = $("#screen");
    vnjs.once('postload', function () {
      var scale = vnjs["package"].scale;
      if (!scale || scale === 1) return;
      document.querySelector('.wrapper').style.transform = "scale(".concat(scale, ")");
    });
  }
    ...
    _createClass(Controller, [{
      ...
      value: function set(data) {
        ...
        var _loop = function _loop(key) {
          ...
          _classPrivateFieldSet(_this, _value, value);
          ...
          if (value.includes("+=")) {
            ...
            _this.valueIncrement(key, val);
          } else if (value.includes("-=")) {
            var _val2 = value.replace("-=", "");

            _this.valueDecrement(key, _val2);
          } else {
            if (isNaN(value)) {
          ...
          if (_valueVar) {
            _valueVar.forEach(function (_varData) {
              var _key2 = _varData.replace("{{", "").replace("}}", "");

              var _value2 = vnjs.state.data[_key2];

              if (!isNaN(_value2)) {
                _value2 = Number(_value2);
              }

              _classPrivateFieldSet(_this, _value, _classPrivateFieldGet(_this, _value).replaceAll(_varData, _value2));

              vnjs.state.data[key] = _classPrivateFieldGet(_this, _value);
            });
          }
        };
      ...
      key: "stringToData",
      value: function stringToData(reply) {
        var _newReply = String(reply);

        var variables = String(reply).match(/{{.+?}}/g);
  ...
  var controller$3 = new Controller();

  function data () {
    vnjs.plugins['data'] = {
      stringToData: controller$3.stringToData
    };
  }
  /**
   * events
  ...
  vnjs.on("postload", function () {
    controller$3.load(vnjs["package"].publish.token);
  });
  vnjs.on("data-set", function (args) {
    return controller$3.set(args);
  });
  vnjs.on("data-clear", function (args) {
    return controller$3.clear(args);
  });
  vnjs.on("data-save", function () {
    controller$3.save(vnjs.state.data);
  });
  /**
   * deprecated
   */
  ...
  var DialogBox = /*#__PURE__*/function () {
    ...
    _createClass(DialogBox, [{
      ...
      value: function print(character) {
        ...
        this.reply = vnjs.plugins['data'].stringToData(_classPrivateFieldGet(this, _reply)); //вырезаем пустые теги
  ...
  var dBox = null;
  function dialogBox () {
    vnjs.store.screen.append($tpl$j);
    dBox = new DialogBox({
      delay: 0,
      alpha: 0,
      endPoint: false,
    ...
    $tpl$j.find(".dialog-box__reply-wrapper").on("mousedown", function (e) {
    ...
    if (conf) {
      for (var key in conf) {
        switch (key) {
          ...
          case "font-size":
            $tpl$j.css("font-size", conf["font-size"]);
            break;

          case "line-height":
            $tpl$j.css("line-height", conf["line-height"]);
            break;
  ...
  function menu () {
    ...
    var menu = new Menu$1($tpl$i, this, config);
    /**
     * Навешиваем click на пункты меню
     */

    $tpl$i.on("click", ".vnjson__menu-item", function () {
      var label = $(this).data('label');
      menu.clickItemHundler(label);
    });
    vnjs.store.screen.append($tpl$i);
    /**
     * аудио
     */
  ...
  var tpl$b = "<div class=\"main-menu component\"></div>";
  ...
  function mainMenu () {
    ...
    var menu = new Menu($tpl$h, this, config);
    /**
     * Навешиваем click на пункты меню
     */

    $tpl$h.on("click", ".main-menu__item", function () {
      var label = $(this).data('label');
      menu.clickItemHundler(label);
    });
    vnjs.store.screen.append($tpl$h);
    /**
     * аудио
     */
  ...
  var css$o = ".vnjson-scene{\n    background-repeat: no-repeat;\n    background-size: cover;\n    z-index: 0;\n    width: 100%;\n    height: 100%\n}";
  n(css$o,{});

  var $tpl$g = $('<div class="vnjson-scene component"></div>');
  function scene () {
    vnjs.store.screen.append($tpl$g);
    vnjs.on("scene", function (id) {
      vnjs.emit('image.update', $tpl$g, {
        plugin: 'scene',
  ...
  */
    ...
    _createClass(Switch, [{
      ...
      value: function parse(data) {
        ...
        for (var equal in this.PLUGIN_DATA) {
          ...
          controller$2[this.OPERATOR](this.dataValue, this.value, execData);
        }
      }
    }]);
  ...
  function input$1 () {
    ...
    $tpl$f.find(".vnjson__input-btn").on("click", function () {
      return clickHandler();
    });
  }
  vnjs.on("input-name", handler$3);
  vnjs.on("input-data", handler$3);
  /**
   * deprecated
   */
  ...
  var css$l = ".vnjson__wiki {\n  left: 10px;\n  top: 40px;\n  width: 300px;\n  height: 400px;\n  display: none;\n  padding: 10px;\n  border-radius: 4px;\n  background-color: rgba(0,0,0,0.5);\n  font-size: 14px;\n  overflow: auto;\n}\n.vnjson__wiki li{\n  padding: 5px 0;\n\n  line-height: 18px;\n    \n}\n.vnjson__wiki-key{\n  color: wheat;\n}\n.vnjson__wiki-dash{\n  padding: 0 5px;\n  color: silver;\n}\n.vnjson__wiki-value{\n  color: darkgray;\n}";
  n(css$l,{});

  var $tpl$e = $('<ul class="vnjson__wiki component"></ul>');
  function wiki () {
    var _this = this;
  ...
  var css$k = ".screen-vnjson__slide{\n  position: absolute;\n  display: none;\n  top: 5%;\n  left: 50%;\n  transform: translateX(-50%);\n  box-shadow: 3px 3px 12px rgba(0, 0, 0, 0.5);\n  z-index: 2000;\n}";
  n(css$k,{});

  var $tpl$d = $("<img class=\"screen-vnjson__slide component\"/>");
  function slide () {
    ...
    vnjs.on('slide', function (id) {
      ...
      if (typeof id === 'string') {
        $tpl$d.attr('src', vnjs.getAssetByName(id).url).show();
        ...
        if (id.hasOwnProperty('css')) {
          $tpl$d.css(id.css);
          $tpl$d.attr('src', id.name).show();
        }

        if (id.hasOwnProperty('url')) {
          $tpl$d.attr('src', id.url).fadeIn();
        } else {
          $tpl$d.attr('src', vnjs.getAssetByName(id.name).url).show();
        }
        /*
        .css(id.css)
         */
  ...
  var $tpl$c = $("<ul class=\"vnjson__stage component\"></ul>");
  function content () {
    var content = new Content(this, $tpl$c);
    ...
    $tpl$c.on('click', '.stage-item', function () {
      var label = $(this).data('label');
  ...
  var Test = /*#__PURE__*/function () {
    ...
    _createClass(Test, [{
      ...
      value: function renderQuetion() {
        var _this$_quetionItem, _this$_quetionItem2;
        ...
        if ((_this$_quetionItem = this._quetionItem) !== null && _this$_quetionItem !== void 0 && _this$_quetionItem.reply) {
          $(".dialog-box__reply").html(this._quetionItem.reply);
        } else {
          $(".dialog-box__reply").html("");
        }

        if ((_this$_quetionItem2 = this._quetionItem) !== null && _this$_quetionItem2 !== void 0 && _this$_quetionItem2.audio) {
      ...
      value: function nextStep() {
        ...
        setTimeout(function () {
          ...
          if (_this.qIndex === _this.args.questions.length) {
            _this.onEnd();
          } else {
            _this.renderQuetion();
          }
      ...
      value: function onEnd() {
        ...
        var _vnjs$state$data = vnjs.state.data,
            ...
            summTest = _vnjs$state$data.summTest;
          ...
          if (index === this._quetionItem.correct - 1) {
            ...
            if (this.args["self-control"]) {
              ...
              $(".vnjson__variants-item").toArray().map(function (item) {
                if ($(item).data("index") === _this2.args.questions[_this2.qIndex].correct - 1) {
  ...
  $tpl$b.find(".vnjson__test-variants").on("click", ".vnjson__variants-item", function () {
    test.clickItem(this);
  });
  $tpl$b.find(".vnjson__test-next-btn").on("click", function () {
    vnjs.exec({
      next: true,
      test: false
    });
  ...
  function mcExec () {
    vnjs.on('mc-exec', function (param) {
      var _data = vnjs.plugins.data.stringToData(param.data);
  ...
  function mcGet$1 (args) {
    var _pos = vnjs.plugins['data'].stringToData(args.pos);

    var _pos$split = _pos.split(" "),
        _pos$split2 = _slicedToArray(_pos$split, 3),
        x = _pos$split2[0],
        y = _pos$split2[1],
        z = _pos$split2[2];
  ...
  function mcGetBlockCommon(args) {
    var block = vnjs.store.MINECRAFT.BLOCK.data;
    var props = ["age", "alt", "attached", "axis", "bites", "check_decay", "color", "contents", "decayable", "delay", "disarmed", "east", "enabled", "extended", "eye", "facing", "flip", "half", "has_bottle_0", "has_bottle_1", "has_bottle_2", "has_record", "hinge", "in_wall", "legacy_data", "level", "locked", "mode", "moisture", "nodrop", "north", "occupied", "open", "part", "power", "powered", "rotation", "seamless", "shape", "short", "snowy", "south", "suspended", "triggered", "type", "up", "upper", "variant", "west"];
    var idExist = false;
    ...
    for (var key in args) {
      ...
      if (props.includes(key)) {
        for (var blockKey in block.state) {
          if (blockKey.includes("name=".concat(key))) {
            var blockParam = block.state[blockKey];
  ...
  function mcGetBlockNBT(args) {
    var block = vnjs.store.MINECRAFT.BLOCK;

    for (var key in args) {
      var value = block.data.nbt[key];
  ...
  function mcGetStoreSlot(args) {
    var slot = args.slot;
    var block = vnjs.store.MINECRAFT.BLOCK;
    var items = block.data.nbt.Items;
  ...
  function mcGetStoreTotal(args) {
    var block = vnjs.store.MINECRAFT.BLOCK;
    var hand = vnjs.store.MINECRAFT.HAND;
    var items = null;
    ...
    if (args.formula === "SumRateCount") {
      ...
      _items.forEach(function (_i) {
        var rate = args.rates.filter(function (r) {
          var rateDamage = Number(Object.keys(r)[0]);
          return rateDamage === _i.Damage;
        })[0];

        if (rate) {
          var val = Object.values(rate)[0];
          sum += val * _i.Count;
        }
      });
    ...
    if (args.formula === "SumCount") {
      ...
      vnjs.state.data[args.result] = _sum;
      console.log(_sum);
    }
  }
  /**
  ...
  function mcGetSign(args) {
    var block = vnjs.store.MINECRAFT.BLOCK;
    var _block$data$nbt = block.data.nbt,
        Text1 = _block$data$nbt.Text1,
        Text2 = _block$data$nbt.Text2,
        Text3 = _block$data$nbt.Text3,
        Text4 = _block$data$nbt.Text4;

    var _strings = [Text1, Text2, Text3, Text4].map(function (item) {
  ...
  function mcGetFrameItem(args) {
    var block = vnjs.store.MINECRAFT.ENTITY;
    var excludeProps = ["default", "damage"];
    var _block$data$nbt$Item = block.data.nbt.Item,
        id = _block$data$nbt$Item.id,
        Damage = _block$data$nbt$Item.Damage;
    /**
     * Получаю id предмета
     */
  ...
  function mcGetHandItem(args) {
    var _vnjs$store$MINECRAFT = vnjs.store.MINECRAFT.HAND.data,
        id = _vnjs$store$MINECRAFT.id,
        Damage = _vnjs$store$MINECRAFT.Damage;
    var excludeProps = ["default", "damage"];
    /**
     * Получаю id предмета
     */
  ...
  function mcGetBook(args) {
    var book = {};
    vnjs.state.data.pageNumber = 0;
    var _vnjs$store$MINECRAFT2 = vnjs.store.MINECRAFT,
        ENTITY = _vnjs$store$MINECRAFT2.ENTITY,
        HAND = _vnjs$store$MINECRAFT2.HAND;
    ...
    var commonText = "";
    var _book$tag = book.tag,
        author = _book$tag.author,
        title = _book$tag.title,
        pages = _book$tag.pages;
    /**
     * Завершённая книга
     */
    ...
    if (book.id === 'minecraft:written_book') {
      pages.forEach(function (page, index) {
        ...
        var _strings = JSON.parse(page).text.replaceAll('\n', '<br/>');

        commonText += "".concat(_strings, "<br/>").concat(pageBreak);
  ...
  function mcGetBookPage(args) {
    var book = vnjs.store.MINECRAFT.HAND.data;
    ...
    var _book$tag2 = book.tag;
        _book$tag2.author;
        _book$tag2.title;
        var pages = _book$tag2.pages;
    var PAGE = pages[args.page - 1];
    console.log(PAGE);

    if (!PAGE) {
      vnjs.emit('vnjson.error', "\u0421\u0442\u0440\u0430\u043D\u0438\u0446\u044B \u2116 <font color=\"deepskyblue\">".concat(args.page, "</font> \u043D\u0435 \u0441\u0443\u0449\u0435\u0441\u0442\u0432\u0443\u0435\u0442"));
      return;
    }

    var pageBodyJSON = PAGE;
    var pageBody = JSON.parse(pageBodyJSON).text.replaceAll('\n', '<br/>');
  ...
  function mcGetPlayerNBT(args) {
    var PLAYER = vnjs.store.MINECRAFT.PLAYER;
    var playerData = PLAYER.data.player;
    var playerEffects = playerData.playerEffects;

    var commonData = _objectSpread2(_objectSpread2({}, playerData), playerEffects);

    for (var key in args) {
      var value = commonData[key];
  ...
  function mcGetCardItem (args) {
    var TYPE = vnjs.store.MINECRAFT.TYPE;
    var ITEM = vnjs.store.MINECRAFT[TYPE];
    var card = null;

    switch (ITEM.data.id) {
      case 'is_mtc:item_binder':
        card = ITEM.data.tag.Items.Items.find(function (item) {
          return "".concat(item.tag.cdwd, " ").concat(item.tag.assetnumber) === args.card;
        });
        break;

      case 'is_mtc:block_displayer':
        card = ITEM.data.nbt.Items.Items.find(function (item) {
          return "".concat(item.tag.cdwd, " ").concat(item.tag.assetnumber) === args.card;
        });
        break;
   ...
   * mc-get-card-slot
   */
  function mcGetCardSlot (args) {
    var IDs = ['is_mtc:block_monodisplayer', 'is_mtc:block_displayer', 'is_mtc:item_card_common'];
    var TYPE = vnjs.store.MINECRAFT.TYPE;
    var ITEM = vnjs.store.MINECRAFT[TYPE];
    /**
    ...
    var _card$tag = card.tag,
        assetnumber = _card$tag.assetnumber,
        cdwd = _card$tag.cdwd;
    /**
     * Созраняем cardID в переменню
     */

    vnjs.state.data[args.card] = "".concat(cdwd, " ").concat(assetnumber);
    /**
  ...
  function mcGetCardInfo (args) {
    var mtcs = vnjs.tree.$root.mtcs;
   ...
   * mc-get-message
   */
  var excludeProps = ['prefix', 'last'];
  var flag = false;
  function mcGetChat (args) {
    var WORLD = vnjs.store.MINECRAFT.WORLD;
    ...
    var messages = JSON.parse(WORLD.chatHistory);
  ...
  var controller$1 = {
    '===': function _(wordtime, value, execData, mode) {
      if (mode === "worddate") {
        var worddate = new Date(wordtime * 100000).toLocaleDateString();
        var val = new Date(value * 100000).toLocaleDateString();
    ...
    '>': function _(wordtime, value, execData) {
      if (wordtime > value) {
        vnjs.exec(execData);
      }
    },
  ...
  var _wordTime = /*#__PURE__*/new WeakMap();

  var _wordDate = /*#__PURE__*/new WeakMap();

  var Time$1 = /*#__PURE__*/function () {
    function Time(mode) {
      ...
      _classPrivateFieldInitSpec(this, _wordTime, {
        writable: true,
        value: null
      });

      _classPrivateFieldInitSpec(this, _wordDate, {
        writable: true,
        value: null
      });
    ...
    _createClass(Time, [{
      ...
      value: function parse(data) {
        ...
        for (var equal in this.PLUGIN_DATA) {
          ...
          var _this$equal$split = this.equal.split(this.OPERATOR),
              _this$equal$split2 = _slicedToArray(_this$equal$split, 2);
              _this$equal$split2[0];
              var val = _this$equal$split2[1];
      ...
      key: "timeEval",
      value: function timeEval(val) {
        /**
         * Определяем диапазон ли лежит в значении
         */
        if (val.includes('--')) {
          this.value = val.split('--').map(function (item) {
        ...
        var execData = this.PLUGIN_DATA[this.equal];
        controller$1[this.OPERATOR](this.wordtime, this.value, execData);
      }
    }, {
      key: "wordtime",
      get: function get() {
        var _classPrivateFieldGet2 = _classPrivateFieldGet(this, _wordTime).split("-"),
            _classPrivateFieldGet3 = _slicedToArray(_classPrivateFieldGet2, 2),
            hh = _classPrivateFieldGet3[0],
            mm = _classPrivateFieldGet3[1];

        return Number(hh + mm);
      },
      set: function set(val) {
        _classPrivateFieldSet(this, _wordTime, val);
      }
    }, {
      key: "dateEval",
      value: function dateEval(val) {
        var _this = this;
        ...
        */
        if (val.includes('--')) {
          this.value = val.split('--').map(function (item) {
            return _this.transformDate(item.trim());
          });
        } else {
        ...
        var execData = this.PLUGIN_DATA[this.equal];
        controller$1[this.OPERATOR](this.worddate, this.value, execData, this.mode);
      }
    }, {
      key: "worddate",
      get: function get() {
        var _classPrivateFieldGet4 = _classPrivateFieldGet(this, _wordDate).split("-"),
            _classPrivateFieldGet5 = _slicedToArray(_classPrivateFieldGet4, 3),
            yyyy = _classPrivateFieldGet5[0],
            mm = _classPrivateFieldGet5[1],
            dd = _classPrivateFieldGet5[2];

        return Number(yyyy + mm + dd); //const d = this.#wordDate.split()
        //return // this.transformDate();
      },
      set: function set(val) {
        _classPrivateFieldSet(this, _wordDate, val);
      }
    }, {
      key: "transformDate",
  ...
  var date = new Time$1('worddate');
  function mcGetDate (args) {
    var WORLD = vnjs.store.MINECRAFT.WORLD;
  ...
  var time = new Time$1('wordtime');
  function mcGetTime (args) {
    var WORLD = vnjs.store.MINECRAFT.WORLD;
   ...
   * mc-get-print-pos
   */
  function mcGetPrintPos (args) {
    var IDs = ['computercraft:printout'];
    var TYPE = vnjs.store.MINECRAFT.TYPE;
    var ITEM = vnjs.store.MINECRAFT[TYPE];
    ...
    var _ITEM$data$tag = ITEM.data.tag,
        title = _ITEM$data$tag.title;
        _ITEM$data$tag.pages;
    ...
    for (var key in ITEM.data.tag) {
      if (key.includes('line')) {
        var value = ITEM.data.tag[key].trim();

        var _key = key.replace('line', '');

        lines[_key] = value;
      }
    }

    var excludeProps = ['title', 'page', 'string', 'start', 'length', 'default'];
    var _pages = [];
    var countStr = 0;
    var page = 0;
    ...
    for (var _key2 in args) {
      if (!excludeProps.includes(_key2)) {
        var str = _pages.find(function (item) {
          if (item.page === args.page - 1 && item.string === args.string - 1) {
            return true;
          }
        });

        var chunk = str.line.split('').splice(args.start - 1, args.length).join("");

        if (chunk === _key2) {
          flag = true;
          vnjs.exec(args[_key2]);
        }
      }
    }
  ...
  function mcGetPrintLabel (args) {
    ...
    var excludeProps = ['title', 'label', 'default'];
    var flag = false;
  ...
  */
  ...
  function mcGetEntity (args) {
    ...
    for (var key in args) {
      switch (key) {
        ...
        case "pos":
          var _ITEM$data$nbt$Pos = _slicedToArray(ITEM.data.nbt.Pos, 3),
              x = _ITEM$data$nbt$Pos[0],
              z = _ITEM$data$nbt$Pos[1],
              y = _ITEM$data$nbt$Pos[2];

          vnjs.state.data[args.pos] = "".concat(Math.floor(x), " ").concat(Math.floor(z), " ").concat(Math.floor(y));
          break;
        ...
        default:
          vnjs.emit('vnjson.error', "\u0421\u0432\u043E\u0439\u0441\u0442\u0432\u043E [ ".concat(key, " ] \u043D\u0435\u043F\u043E\u0434\u0434\u0435\u0440\u0436\u0438\u0432\u0430\u0435\u0442\u0441\u044F"));
      }
    }
  }
  ...
  vnjs.on("mc-get", mcGet$1);
  /**
   * сравнение по ID
   */

  vnjs.on("mc-get-block", mcGetBlockCommon);
  /**
   * nbt
   */

  vnjs.on("mc-get-block-nbt", mcGetBlockNBT);
  /**
   * get chest
   */

  vnjs.on("mc-get-store-slot", mcGetStoreSlot);
  /**
   * get chest total items
   */

  vnjs.on("mc-get-store-total", mcGetStoreTotal);
  /**
   * табличка
   */

  vnjs.on("mc-get-sign", mcGetSign);
  /**
   * Рамка
   */

  vnjs.on('mc-get-frame-item', mcGetFrameItem);
  /**
   * Предметы в руках
   */

  vnjs.on('mc-get-hand-item', mcGetHandItem);
  /**
   * book
   */

  vnjs.on('mc-get-book', mcGetBook);
  vnjs.on('mc-get-book-page', mcGetBookPage);
  /**
   * player
   */

  vnjs.on('mc-get-player-nbt', mcGetPlayerNBT);
  /**
   * card
   */
  ...
  function hands () {
    ...
    vnjs.on("vnjson.character", function (character) {
      return characterHandler(vnjs.state.character);
    });
  }
  ...
  function leftHandler(args) {
    if (args) {
      show = true;
      $tpl$a.show();
      $tpl$a.find(".vnjson__hand-left").css("background-image", "url('".concat(vnjs.getAssetByName(args).url, "')"));
    } else {
  ...
  function rightHandler(args) {
    if (args) {
      show = true;
      $tpl$a.show();
      $tpl$a.find(".vnjson__hand-right").css("background-image", "url('".concat(vnjs.getAssetByName(args).url, "')"));
    } else {
  ...
  var tpl$6 = "<div class=\"vnjson__video component\">\n    <video id=\"vnjson__video\" class=\"video-js\"></video>\n</div>\n";
  ...
  var $tpl$9 = $('<div class="vnjson__area component"></div>');
  var _regions = null;
  var onClickData = null;
  var onClickOutData = null;
  function area () {
    var _this = this;

    vnjs.store.screen.append($tpl$9);
    vnjs.on("area", handler$1.bind(this));
    $tpl$9.on("click", function (e) {
  ...
  $tpl$9.on('click', function (e) {
    var areaItem = e.target.className.includes('vnjson__area-item');
    if (areaItem) return;
  ...
  var $tpl$8 = $('<div class="vnjson__blocks component"></div>');
  var stepsArray = [];
  var vnjs$1 = null;
  function blocks () {
    var _this = this;
    ...
    vnjs$1 = this;
    vnjs$1.store.screen.append($tpl$8);
  ...
  function replaceData(str) {
    return vnjs$1.plugins['data'].stringToData(str);
  }
  ...
  var css$9 = ".vnjson__sprites-animate{\n    background-repeat: no-repeat;\n}";
  n(css$9,{});
  ...
  var controller = {
    '===': function _(localtime, value, execData, mode) {
      if (mode === "localdate") {
        var localdate = new Date(localtime * 100000).toLocaleDateString();
        var val = new Date(value * 100000).toLocaleDateString();
    ...
    '>': function _(localtime, value, execData) {
      if (localtime > value) {
        vnjs.exec(execData);
      }
    },
  ...
  var Time = /*#__PURE__*/function () {
    ...
    _createClass(Time, [{
      ...
      key: "localtime",
      get: function get() {
        var _Date$toTimeString$sp = new Date().toTimeString().split(":"),
            _Date$toTimeString$sp2 = _slicedToArray(_Date$toTimeString$sp, 2),
      ...
      key: "getEasterWeek",
      value: function getEasterWeek() {
        var years = [{
          year: "2022",
          range: [16507584, 16512768]
        }, // 2022-04-24 --> 2022-04-30
        {
          year: "2023",
          range: [16816032, 16821216]
        }, // 2023-04-16 --> 2023-04-22
        {
          year: "2024",
          range: [17148672, 17153856]
        }, // 2024-05-05 --> 2024-05-11
        {
          year: "2025",
          range: [17451072, 17456256]
        ...
        var currentYear = new Date(this.localdate * 100000).toLocaleDateString();
        /**
         * Ищую соответсвие ключе year с годом в текущей дате
         */

        var _year = years.find(function (date) {
          return currentYear.includes(date.year);
        });
  ...
  function getLocalTime () {
    var time = new Time('localtime');
    var date = new Time('localdate');
    vnjs.on('get-localtime', function (args) {
      return time.parse(args);
    });
    vnjs.on('get-localdate', function (args) {
      return date.parse(args);
    });
  }

  var intervalVnjson = (function () {
    var intervalID = null;
    vnjs.on('interval', function (args) {
      if (_typeof(args) === "object") {
  ...
  function formatURL(base, query) {
    return "".concat(base, "/query?q=").concat(encodeURIComponent(query));
  }

  function getData () {}
  vnjs.on('get-uuid', function (varName) {
    vnjs.state.data[varName] = uuidv4();
  });
  vnjs.on('get-rnd', function (args) {
    var rnd = getRandom(args.min, args.max);
  ...
  vnjs.on('get-data', function (args) {
    if (vnjs["package"]['url-db']) {
      var id = vnjs.plugins.data.stringToData(args.id);
      var namespace = vnjs.plugins.data.stringToData(args.namespace);
      var query = "SELECT * FROM ".concat(namespace, " WHERE id = '").concat(id, "'");
      var URL = formatURL(vnjs["package"]['url-db'], query);
  ...
  vnjs.on('get-sum', function (args) {
    var sum = 0;
    args.values.forEach(function (item) {
      var _item = Number(vnjs.plugins.data.stringToData(item));

      sum = sum + _item;
    });
    vnjs.state.data[args.sum] = sum;
  });
  vnjs.on('get-str', function (args) {
    var sum = "";
    args.values.forEach(function (item) {
      var _item = vnjs.plugins.data.stringToData(item).trim();
  ...
  function hudInvHandler(args) {
    var $slot = $tpl$7.find(".vnjson-hud__item--".concat(args.slot));
  ...
  var actions = {
    create: function create(args) {
      ...
      $tpl$6 = $("<div id=\"".concat(prefix).concat(args.id, "\" class=\"").concat(prefix, "player\"></div>"));
      ...
      createIFrame(args.id, args.css.width, args.css.height, $tpl$6);
    },
    hide: function hide() {
      $tpl$6.hide();
    },
  ...
  var $tpl$4 = $('<div class="drag-items component"></div>');
  function dragItems () {
    var _this = this;
  ...
  function renderItems(data) {
    ...
    var _data = _toConsumableArray(data.items);

    var maxLength = _data.sort(function (a, b) {
      return b.length - a.length;
    })[0].length;

    var maxWidth = 10 * maxLength;
  ...
  function handler(scriptData) {
    if (scriptData) {
      ...
      $tpl$3.find(".select-word__variants").empty();
  ...
  var tpl$1 = "<iframe id=\"vnjson__paint-board\" src=\"data/board/index.html\" width=\"800\" height=\"500\"></iframe>";

  var css$2 = "#vnjson__paint-board{\n  top: 50px;\n  left: 50%;\n  transform: translateX(-50%);\n  border: 0;\n  display: none;\n  box-shadow: 3px 3px 5px rgba(0,0,0,0.3);\n}";
  n(css$2,{});

  var $tpl$2 = $(tpl$1);
  function paintBoard () {
    vnjs.store.screen.append($tpl$2);
  }
  vnjs.on('paint-board', function (imgID) {
    if (vnjs["package"]['paint-board'] && imgID) {
      $tpl$2.show();
      var url = vnjs.getAssetByName(imgID).url.split('');
      url.shift();
      var IMG = location.protocol + '//' + location.host + '/s' + url.join('');
      var win = document.querySelector("iframe#vnjson__paint-board").contentWindow;
      win.postMessage({
        IMG: IMG
      }, location.origin + "/data/board/index.html");
    } else {
      $tpl$2.fadeOut();
    }
  });

  var tpl = "<iframe id=\"vnjson__chess\" src=\"data/chess/index.html\" width=\"500\" height=\"500\"></iframe>";

  var css$1 = "#vnjson__chess{\n  top: 50px;\n  left: 50%;\n  transform: translateX(-50%);\n}";
  n(css$1,{});
  ...
  vnjs.on('chess-board', function (fen) {
    if (vnjs["package"]['chess-board'] && fen) {
      $tpl$1.show();
      var win = document.querySelector("iframe#vnjson__chess").contentWindow;
      win.postMessage({
        fen: fen
  ...
  var css = ".vnjson__brython{\n  top: 45px;\n  left: 50%;\n  width: 810px;\n  height: 480px;\n  overflow: hidden;\n  transform: translateX(-50%);\n  background-image: url(data:image/png;base64,);\n  background-size: contain;\n  background-repeat: no-repeat;\n  background-position: center;\n}\n#vnjson__brython{\n  top: 23px;\n  left: 50%;\n  width: 758px;\n  height: 430px;\n  overflow: hidden;\n  transform: translateX(-50%);\n}";
  n(css,{});

  var $tpl = $('<div class="vnjson__brython component"></div>');
  function brythonIDE () {
    vnjs.on('postload', function () {
  ...
  function init() {
    ...
    var $iframe = $('<iframe id="vnjson__brython" src="data/pyide/index.html" width="758" height="430"></iframe>');
    vnjs.store.screen.append($tpl);
    $tpl.append($iframe);
    vnjs.on('brython', function (data) {
      if (data) {
        $tpl.fadeIn();

        var file = _this.getDataByName(data);

        var win = document.querySelector("iframe#vnjson__brython").contentWindow;
        var url = location.origin + "/data/pyide/index.html";
  ...
  vnjs.use(getData);
  vnjs.use(HUD); // Zimjs, Typewrite, Show Frame,
  ...
  vnjs.use(paintBoard);
  vnjs.use(chessBoard);
  vnjs.use(brythonIDE); // Load scenes
